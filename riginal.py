...
def ShiftRow(np_array):
    ...
    左シフト
    """
    arrary_list = [ np.roll(np_array[j],-j).tolist() if j!=0 else np_array[j] for j in range(len(np_array))]
    shift_array = np.array(arrary_list)
    return shift_array

def InvShiftRow(np_array):
    """
    右シフト
    """
    arrary_list = [ np.roll(np_array[j],j).tolist() if j!=0 else np_array[j] for j in range(len(np_array))]
...
def SubBytes(plain_array):
    for j in range(4):
        for i in range(4):
            strip_bin = bin(plain_array[j][i])[2:]
            padding = 8-len(strip_bin)
            bits = "0"*padding + strip_bin
            up_number = int(bits[:4],2)
            down_number = int(bits[4:],2)
            plain_array[j][i] = sbox_array[up_number,down_number]
    return plain_array

def InvSubBytes(plain_array):
    for j in range(4):
        for i in range(4):
            strip_bin = bin(plain_array[j][i])[2:]
            padding = 8-len(strip_bin)
