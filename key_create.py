...
class Key_Create():
    ...
    def sub_bytes(self,plain_array):
        for i in range(4):
            strip_bin = bin(plain_array[i])[2:]
            padding = 8-len(strip_bin)
            bits = "0"*padding + strip_bin
            up_number = int(bits[:4],2)
            down_number = int(bits[4:],2)
            plain_array[i] = self.sbox[up_number,down_number]

        return plain_array


    def row_word(self,row_data):
        row_change = np.roll(row_data,-1)
        return row_change
    ...
    def create_key(self,bit_len):
        ...
        key_array = np.array(tmp_list)
        round_key = np.reshape(key_array,(4,4))
        return round_key

    def xor_three(self,origin,words,recon):
        result = []
        for j in range(len(origin)):
            result.append(origin[j]^words[j]^int(recon[j],16))

        return result

    def xor_two(self,origin,words):
        result = []
        for j in range(len(origin)):
            result.append(origin[j]^words[j])
